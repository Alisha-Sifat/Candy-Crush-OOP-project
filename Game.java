...
public class Game {
        ...
        if (selected == null) {
            ...
            if (isNeighbor(selected, second)) {
                ...
                if (matches.isEmpty()) {
                    ...
                    // ⭐ REMOVE special candy from match list
                    ...
                    // ⭐ ACTIVATE SPECIAL CANDIES (🔥 FIX)
                    for (Point p : matches) {
                        Candy c = board.get(p.x, p.y);

                        if (c instanceof StripedCandy) {
                            board.clearRowOrColumn(p, (StripedCandy) c);
                        }
                        else if (c instanceof BombCandy) {
                            int targetType = board.get(selected.x, selected.y).getType();
                            board.clearColor(targetType);
                        }
                    }
    ...
    private void processCascade() {
        ...
        if (!matches.isEmpty()) {
            ...
            // 🔥 ALSO activate specials in cascade
            for (Point p : matches) {
                Candy c = board.get(p.x, p.y);
